...
def _read_effective_index_with_height(file_name: str) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    """Reads effective index data from file.
    ...
    Returns:
        tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]: Effective index data, width data, wavelength data, height data.
    """
    ...
    _width_data = jnp.array(list(map(float, _lines[3][:-2].split(","))))
    _wav_data = jnp.array(list(map(float, _lines[5][:-2].split(","))))
    _height_data = jnp.array(list(map(float, _lines[7][:-2].split(","))))

    _wav_size = _wav_data.shape[0]
    _width_size = _width_data.shape[0]
    _height_size = _height_data.shape[0]

    _neff_data = _neff_data.reshape((_height_size, _width_size, _wav_size))
    return _neff_data, _width_data, _wav_data, _height_data
