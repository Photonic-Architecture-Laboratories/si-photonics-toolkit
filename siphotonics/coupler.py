...
def plot_power_output(transfer_matrix, wavelength, device_length, kind="frequency_sweep"):
    ...
    if kind=="device_length":
        ...
        sample_point_number = transfer_matrix.shape[1]
        ...
        for i in range(wavelength.shape[0]):
            for j in range(sample_point_number):
                t[i, j] = np.dot(transfer_matrix[i, j, :, :], input)[0][0]
                q[i, j] = np.dot(transfer_matrix[i, j, :, :], input)[1][0]
        ...
        for i in range(wavelength.shape[0]):
                t[i, 0] = np.dot(transfer_matrix[i, -1, :, :], input)[0][0]
                q[i, 0] = np.dot(transfer_matrix[i, -1, :, :], input)[1][0]
